        ...
        >
            <div className="flex flex-col md:flex-row justify-between items-end px-4 md:px-8 mb-8 gap-4">
                <div className="relative">
                    <h2 className="text-2xl md:text-4xl font-black text-[#1A2036] uppercase tracking-wider relative z-10">
                        {title}
                    </h2>
                    <div className="absolute -bottom-2 left-0 w-2/3 h-3 bg-[#B5224E]/20 -skew-x-12 -z-0"></div>
                </div>

                <div className="flex gap-3">
                    <button 
                        onClick={() => scroll('left')} 
                        className="w-12 h-12 rounded-full border-2 border-[#1A2036] flex items-center justify-center text-[#1A2036] hover:bg-[#1A2036] hover:text-white transition-all duration-300 active:scale-95 shadow-md hover:shadow-lg"
                    >
                    ...
                    return (
                        <div 
                            ...
                            className={`
                                ...
                                ${isActive 
                                    ? 'scale-100 opacity-100 shadow-[0_20px_40px_-10px_rgba(31,41,55,0.4)] z-20 ring-4 ring-[#B5224E] ring-offset-4 ring-offset-white' 
                        ...
                        >
                            ...
                            <div className={`absolute inset-0 bg-gradient-to-t from-[#1A2036]/90 via-transparent to-transparent transition-opacity duration-500 ${isActive ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`} />

                            <div className="absolute bottom-0 left-0 w-full p-6 transform transition-transform duration-500">
                                <span className={`inline-block px-3 py-1 bg-[#B5224E] text-white text-[10px] font-bold uppercase tracking-widest rounded-full mb-3 shadow-lg transform transition-all duration-500 delay-100 ${isActive ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0 group-hover:translate-y-0 group-hover:opacity-100'}`}>
                                    {item.category}
                                </span>
...
const WIEGallery: React.FC = () => {
  const [filter, setFilter] = useState('All');
  const [lightboxImage, setLightboxImage] = useState<{src: string, caption: string} | null>(null);

  const filters = ['All', 'Events', 'Social', 'Awards', 'Workshop'];

  const filteredItems = useMemo(() => 
    WIE_GALLERY.filter(item => filter === 'All' || item.category.toLowerCase().includes(filter.toLowerCase())),
  [filter]);

  return (
    <div className="min-h-screen bg-white font-sans pt-32 pb-20 overflow-x-hidden selection:bg-[#B5224E] selection:text-white">
      
      <div className="max-w-7xl mx-auto px-4 text-center mb-16 animate-in fade-in slide-in-from-bottom-8 duration-700">
        <h1 className="text-4xl md:text-6xl font-black text-[#1A2036] mb-6 uppercase tracking-tight">
            WIE <span className="text-[#B5224E] underline decoration-4 underline-offset-4 decoration-[#1A2036]">Memories</span>
        </h1>
        <p className="text-gray-500 max-w-2xl mx-auto text-lg">
            Celebrating the moments that define our community.
        </p>
      </div>
            ...
            {filters.map((f) => (
              <button
                ...
                className={`
                    ...
                    ${filter === f
                        ? 'bg-[#B5224E] text-white shadow-[0_10px_25px_-5px_rgba(181,34,78,0.5)] scale-110 -translate-y-1'
                        : 'bg-white text-gray-500 border border-gray-200 hover:border-[#B5224E] hover:text-[#B5224E] hover:shadow-md'
                    }
                `}
              >
                {f}
      ...
      <div className="max-w-7xl mx-auto space-y-12">
        {filteredItems.length > 0 ? (
            <div className="animate-in fade-in slide-in-from-bottom-8 duration-700 delay-100">
                <GalleryCarousel 
                    title={filter === 'All' ? 'All Moments' : `${filter} Highlights`} 
                    items={filteredItems} 
            ...
            <div className="text-center py-20">
                ...
                <button onClick={() => setFilter('All')} className="text-[#B5224E] font-bold mt-4 hover:underline">View All Photos</button>
            </div>
        )}
      </div>
      ...
      {lightboxImage && (
        <div 
            className="fixed inset-0 z-[100] bg-[#1A2036]/95 backdrop-blur-md flex items-center justify-center p-4 animate-in fade-in duration-300" 
        ...
        >
            ...
            >
                ...
                <div className="mt-6 text-center animate-in slide-in-from-bottom-4 duration-500 delay-100">
                    ...
                    <div className="h-1 w-20 bg-[#B5224E] mx-auto rounded-full"></div>
                </div>
            </div>
        </div>
      )}
